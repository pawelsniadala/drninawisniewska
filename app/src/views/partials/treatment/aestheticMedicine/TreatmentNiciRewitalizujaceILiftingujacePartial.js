...
const TreatmentNiciRewitalizujaceILiftingujacePartial = () => {
    ...
    const filteredSpecialists = team.filter(item => item.treatment.includes('nici-rewitalizujace-i-liftingujace'))
    ...
    const filteredTreatments = treatment.filter(item => item.specialization.includes('aesthetic-medicine')).filter(item => item.treatment !== 'nici-rewitalizujace-i-liftingujace');
